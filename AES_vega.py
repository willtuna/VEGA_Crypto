...
def AddRoundKey(st_arr, key_schedule_w):
...
def SubByte(st_arr):
    ...
    for r in range(4):
        for c in range(4):
            ...
            st_arr[r,c]  = Sbox[int(hex_x_y[0])][int(hex_x_y[1])]
...
def Encryption(input_bin_str, key_schedule_w):
    st_arr = In2StateArray(input_bin_str)

    # 1st Round Add round Key
    AddRoundKey(st_arr,key_schedule_w)
